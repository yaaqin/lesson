...
export default function CategoryChallengesPage() {
  ...
  const categoriesQuery = useCategoriesQuery(params.tierCode);
  const category = categoriesQuery.data?.find((c) => c.id === params.categoryId);
  const challengesQuery = useChallengesByCategoryQuery(params.categoryId);
    ...
    <div className="flex min-h-screen flex-col bg-zinc-50 font-sans dark:bg-black">
      ...
      <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-8 px-6 py-8">
        ...
        {challengesQuery.isLoading && (
          <p className="text-sm text-zinc-500 dark:text-zinc-500">Memuat level…</p>
        )}
        ...
        <div className="flex flex-col gap-3">
          {challengesQuery.data?.map((challenge, index) => (
            ...
            >
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-purple-600 text-sm font-semibold text-white dark:bg-purple-500">
                {index + 1}
              </span>
              <div className="flex flex-1 flex-col">
                <span className="font-semibold text-black dark:text-zinc-50">{challenge.name}</span>
                <span className="text-xs text-zinc-500 dark:text-zinc-500">
                  {challenge.questionCountRequired} puzzle · lulus minimal {challenge.passThresholdPercent}% ·{" "}
              ...
              {challenge.completed ? (
                ...
                <span className="shrink-0 text-sm font-medium text-blue-600 dark:text-blue-400">Mulai →</span>
              )}
            </Link>
          ))}
        </div>
